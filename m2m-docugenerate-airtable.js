const config = input.config({
    title: 'DocuGenerate - Multiple pdfs for multiple records',
    ...
    items: [
        ...
        input.config.select('singleFile', {
            label: 'Single file',
            description: 'Choose if generated documents will be combined in a single file. Options are yes or no',
            options: [
                {label: 'yes', value: 'yes'},
                {label: 'no', value: 'no'},
            ]
        }),
        input.config.select('pageBreak', {
            label: 'Page break',
            description: 'whether to add a page break in the document after each data row (only applies if single_file is set to yes)',
            options: [
                {label: 'yes', value: 'yes'},
                {label: 'no', value: 'no'},
            ]
...
const singleFile = config.singleFile == "yes" ? true : false;
const pageBreak = config.pageBreak == "yes" ? true : false;
...
if (status< 200 || status >= 300){
  ...
  //////// Tags analysis //////
  var record = await input.recordAsync('Pick a record to run the tags analysis. File(s) will be stored on this record', view);
  if (record) {
    ...
    if (!error) {
      ...
      let propagateOptions = await input.buttonsAsync('Would you like to propagate these settings among the view?', ['Yes', 'No']);
    ...
    for (let rec of queryResult.records) {
      if (rec) {
        bulkData.push(await displayTagRules(rec));
      }
    }
